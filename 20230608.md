...
small_dsize = (int(width*0.5), int(height*0.5))
dst1 = cv2.warpAffine(img, m_small, small_dsize)

big_dsize = (int(width*2), int(height*2))
dst2 = cv2.warpAffine(img, m_big, big_dsize)
dst3 = cv2.warpAffine(img, m_big, big_dsize, None, cv2.INTER_CUBIC) # 선명하게 보이는 보간법을 사용

cv2.imshow('original', img)
cv2.imshow('small', dst1)
cv2.imshow('big_cubic', dst2)
cv2.imshow('big', dst3)
